...
public class OSSConfigure {
	...
	public OSSConfigure() throws IOException {
		...
		prop.load(this.getClass().getClassLoader()
				.getResourceAsStream("OSSKey.properties"));

		endpoint = prop.getProperty("ossCliendEndPoint").trim();
		accessKeyId = prop.getProperty("key").trim();
		accessKeySecret = prop.getProperty("secret").trim();
		bucketName = prop.getProperty("bucketName").trim();
		accessUrl = prop.getProperty("ossEndPoint").trim();
	}
