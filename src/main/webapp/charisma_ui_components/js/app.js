...
var App = function () {
    ...
    var emailInit = function () {
    	$('#email_list').dataTable({
            ...
            "oLanguage": { //国际化配置  
                 "sProcessing" : '<img src="/complaint/assets/img/loading-spinner-grey.gif"/><span>&nbsp;&nbsp;正在获取数据，请稍后...</span>', 
        ...
        jQuery('#email_list_wrapper .dataTables_filter input').addClass("form-control input-small input-inline"); // modify table search input
        jQuery('#email_list_wrapper .dataTables_length select').addClass("form-control input-xsmall"); // modify table per page dropdown
        jQuery('#email_list_wrapper .dataTables_length select').select2(); // initialize select2 dropdown 
    }

    var iframeLoaded = function (iframe) {  //获取iframe内嵌页面的高度
    ...
    var reSetIframeHeight = function() {   //重新设置iframe页面高度达到自适应效果
        try {
    ...
    return {
        ...
        init: function () {
            ...
            var sendcount = 0;
	        var completecount = 0;
	        $(window).load(function() {//等到页面所有内容加载完毕后才会触发
	        	reSetIframeHeight();
	        });
	        // 添加ajax全局事件处理。
	        ...
	        }).ajaxComplete(function (e, xhr, opts) { //ajax全部加载完毕时
	            completecount++; 
	            reSetIframeHeight();
	        }).ajaxStop(function () {
	        });
        ...
        blockUI: function (options) {
            ...
            var html = '';
            if (options.iconOnly) {
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '')+'"><img style="" src="/complaint/assets/img/loading-spinner-grey.gif" align=""></div>';
            } else if (options.textOnly) {
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '')+'"><span>&nbsp;&nbsp;' + (options.message ? options.message : 'LOADING...') + '</span></div>';
            } else {    
                html = '<div class="loading-message ' + (options.boxed ? 'loading-message-boxed' : '')+'"><img style="" src="/complaint/assets/img/loading-spinner-grey.gif" align=""><span>&nbsp;&nbsp;' + (options.message ? options.message : 'LOADING...') + '</span></div>';
            }
